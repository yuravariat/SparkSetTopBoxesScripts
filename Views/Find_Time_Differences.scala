...
object Find_Time_Differences {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val cities = List[(String, Long)](
      ("Amarillo", TimeUnit.HOURS.toMillis(0)),
      ("Parkersburg", TimeUnit.HOURS.toMillis(0)),
      ("Little Rock-Pine Bluff", TimeUnit.HOURS.toMillis(0)),
      ("Seattle-Tacoma", TimeUnit.HOURS.toMillis(0)))
    ...
    for (dateToCheck <- datesToCheck) {
      ...
      for (i <- cities.indices) {
        ...
        val devicesSource = "s3n://magnet-fwm/home/TsviKuflik/devices/devices_120_hist_" + cityname + "/*.gz"

        val devices: List[String] = spark.read
          .format("com.databricks.spark.csv")
          .option("header", "true")
          .load(devicesSource)
          .select($"deviceIUniqueD".toString())
          .map(r => r.getString(0))
          .collect().toList
        ...
        val programs: DataFrame = spark.read
          ...
          .withColumn("start_date_time", unix_timestamp(concat($"start_date", $"start_time"), "yyyyMMddHHmmss").cast("Timestamp"))
        ...
        //        println("Average:")
        //        joined
        //          .select(avg($"diffSec").as("diffSec"))
        //          .withColumn("diffTime", MilliToTime($"diffSec"))
        //          .show()
        //
        //        joined.describe("diffSec").show()
