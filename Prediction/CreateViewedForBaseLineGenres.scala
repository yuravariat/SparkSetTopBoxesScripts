package SetTopBoxes.Prediction2
...
/**
  ...
  */
object CreateViewedForBaseLineGenres {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val programs_path = base_path + "y_programs/programs_genres_groups/part-00000"
    ...
    for (i <- cities.indices) {
      ...
      val results_path = base_path + "y_for_prediction/views_for_base/genres/" + city_name + "-" +
      ...
      val views = spark.read
        ...
        .filter(v => v.event_date_time.getTime > predictionStart.getTime && v.event_date_time.getTime < predictionEnd.getTime
          && !notAllowedPRograms.contains(v.prog_code) && programsCodesToGenresIndexArray.contains(v.prog_code))
      ...
      val flated = views.flatMap(v => {
        ...
        times.flatMap(time => {
          val indices = programsCodesToGenresIndexArray(v.prog_code)
          indices.map(genreIndex => (time, (genreIndex, 1)))
        })
      })
