...
object RunProgPredictRawByIndex {
...
//                      ) extends Serializable
  ...
  case class Device(
                     ...
                     prog_rates: Map[Int, (Int, Seq[(Int, Double)])]
                   ) extends Serializable
  ...
  class PredictResultAttemp(
                             ...
                             suggested_progs_group: Seq[Int],
                             suggested_progs_ind: Seq[Int]
                           ) extends Serializable {
  ...
  def main(args: Array[String]): Unit = {
    ...
    for (citykey <- cities.keys) {
      ...
      val rated_programs_by_clusters = clusteredData
        ...
        .map(v => {
          val part = v._1
          //val cluster_num = v._2.head._1
          val vector_headers = vectors_headers(v._2.head._1) // cluster_num -> vector index -> prog index
        ...
        .map { device =>
          ...
          for (view <- views) {
            if (!notAllowedPRograms.contains(view.prog_code)) {
              ...
              val cluster_prog_rates: Seq[(Int, Double)] = rated_programs_by_clusters(pOfDay)(current_clust_num)
              var done = false
              var group_prog_position: Int = 1000
              var index = 0
              val suggested_progs_group:ListBuffer[Int] = ListBuffer[Int]()

              val ratedGroupProgramsContainsThis = cluster_prog_rates.exists(v => v._1 == currentProgramIndex)
              if (ratedGroupProgramsContainsThis) {
                for (i <- cluster_prog_rates.indices; if !done) {
                  if (trans_progs.contains(cluster_prog_rates(i)._1)) {
                    if (cluster_prog_rates(i)._1 == currentProgramIndex) {
                      if (group_prog_position == 1000) { // Recognize first time entrance
                        group_prog_position = index
                        //done = true
                      }
                    }
                    suggested_progs_group.append(cluster_prog_rates(i)._1)
                    index += 1
                    if (index == 21) {
                      done = true
                    }
              ...
              index = 0
              val device_prog_rates: Seq[(Int, Double)] = device.prog_rates(pOfDay)._2
              val ratedProgramsContainsThis = device_prog_rates.exists(v => v._1 == currentProgramIndex)
              val suggested_progs_ind:ListBuffer[Int] = ListBuffer[Int]()

              if (ratedProgramsContainsThis) {
                for (i <- device_prog_rates.indices; if !done) {
                  if (trans_progs.contains(device_prog_rates(i)._1)) {
                    if (device_prog_rates(i)._1 == currentProgramIndex) {
                      if (prog_position == 1000) { // Recognize first time entrance
                        prog_position = index
                        //done = true
                      }
                    }
                    suggested_progs_ind.append(device_prog_rates(i)._1)
                    index += 1
                    if (index == 21) {
                      done = true
                    }
              ...
              val pred_result = new PredictResultAttemp(
                ...
                syncedTime,
                view.prog_code,
                trans_progs.keys.size,
                suggested_progs_group,
                suggested_progs_ind
              )
