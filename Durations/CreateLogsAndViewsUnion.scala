...
/**
  ...
  */
object CreateLogsAndViewsUnion {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val results_path = "s3n://magnet-fwm/home/TsviKuflik/y_views_logs_union"
    ...
    val views = sc.textFile(views_path)
      ...
      (sp(0) + "-" + sp(1), 0, dt, sp(4), sp(5), if (prog.isDefined) prog.get else 0)
    ...
    val logs = sc.textFile(logs_path) // Collect only OFF events
      .filter(s=>{!s.isEmpty && s.split("\\|")(6) == "OFF"})
      .map(s => {
      //0-msoCode
      //1-deviceID
      //2-date
    ...
    views.union(logs)
      .map(l => {
        ...
        //      log_type: LogType, // 0 - view, 1- log
        //      event_date_time: java.sql.Timestamp,
        //      station_num: String,
        //      prog_code: String,
        //      prog_duration: Int
        l._1 + "|" + l._2 + "|" + format.format(l._3) + "|" + l._4 + "|" + l._5 + "|" + l._6
      })
