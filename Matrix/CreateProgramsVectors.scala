...
object CreateProgramsVectors {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val partsOfDay = 1
    val applyTimeDecayFunction = true
    val timeDecayRate = 100.0
    ...
    val programs = sc.textFile(base_path + "y_programs/programs_groups/part*")
      .map(s => {
        val sp = s.split("\\|")
        val prog_name = sp(0)
    ...
    println("Create programs matrix for each city")
    for (cityIndx <- cities.indices) {
      ...
      val devicesRates = spark.read
        ...
        .map(d => {
          ...
          views.foreach(v => {
            val prog_index = progsToIndex.get(v.prog_code)
            if (prog_index.isDefined) {
              val partOfDay = PartsOfDay.GetPartOfDay(partsOfDay, v.event_date_time,div5new)
              var rate = 1.0
              ...
              if (rates(partOfDay).contains(prog_index.get)) {
                rates(partOfDay)(prog_index.get) += rate
              }
              else {
                rates(partOfDay)(prog_index.get) = rate
              }
      ...
      for (pOfDay <- 1 until (partsOfDay + 1)) {
        ...
        val resultsPath = "s3n://magnet-fwm/home/TsviKuflik/y_for_clustering/" + months + "_months" +
        ...
        val programs_indexes_to_include = devicesRates
        ...
        val devicesWV = devicesRates.map(p => {
          val deviceVector: Array[Int] = Array.fill[Int](programs_indexes_to_include.length)(0)
          var vect_index = 0
          for (prog_index <- programs_indexes_to_include) {
            if (p._2(pOfDay).contains(prog_index)) {
              deviceVector(vect_index) = p._2(pOfDay)(prog_index).toInt
            }
            vect_index += 1
          }
